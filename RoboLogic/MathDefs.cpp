...
float CMathDefs::VectorSquaredLength(tVector *v) 
{
	return((v->x * v->x) + (v->y * v->y) + (v->z * v->z));
}

//  下面的函数的功能是求矢量的模，即矢量的长度
float CMathDefs::VectorLength(tVector *v) 
{
...
//  下面的函数的功能是求两个矢量的点积
float CMathDefs::DotProduct(CPoint3D *v1, CVector3D *v2)
{
...
float CMathDefs::VectorSquaredDistance(tVector *v1, tVector *v2) 
{
